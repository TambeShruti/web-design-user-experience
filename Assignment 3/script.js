window.onload = function () {
  var fullName = "Shruti Tambe";
  var nuid = "00276916";
  alert("Welcome, " + fullName + " (NUID: " + nuid + ")");
  var table = document.getElementById("myTable");
  for (var i = 1; i < table.rows.length; i++) {
    addEditButton(table.rows[i]);
  }
};
...
function deleteRow(r) {
  ...
  document.getElementById("myTable").deleteRow(i);
  alert(`Student Record ${Math.floor(rowCount / 2)} Deleted Succesfully`);
  hideOnStart();
  selectRow();
}
...
function editRow(r) {
  ...
  var confirmation = confirm(`
    Edit Details of Student ${Math.floor(rowCount / 2)}
    \n\n
    [Update] [Cancel]
  `);
...
function addRow() {
  ...
  try {
    setTimeout(() => {
      alert(`Student ${Math.ceil(rowCount / 2)} Record added Succesfully`);
    }, 100);
  } catch (error) {
